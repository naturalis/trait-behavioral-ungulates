...
binData <- subset(Responser,select = c("Domestication", "X6.2_TrophicLevel", "Sociality", "SocialHierarchy", "YearRoundBreeding", "DevelopmentStrategy", "HeadOrnaments"))
multiData <- subset(Responser, select = c("X1.1_ActivityCycle", "MatingSystem"))
numData <- subset(Responser, select = -c(Domestication, X6.2_TrophicLevel, Sociality, SocialHierarchy, YearRoundBreeding, DevelopmentStrategy, HeadOrnaments, X1.1_ActivityCycle, MatingSystem))
...
# According to the summary information stored in the txt documents, we choose traits that possess significant predictor variables as well as a R-squared value higher than 0.2 (for numeric traits) as interests to continue to do the research.
sig_binData <- subset(binData,select = c(Sociality, SocialHierarchy, YearRoundBreeding))
sig_numData <- subset(numData, select = c(X22.1_HomeRange_km2,	X15.1_LitterSize, X10.2_SocialGrpSize, AVGFoodConsumption))
# sig_multiData <- subset(multiData, select = c())

# The final models are organized orderly into three new documents:"Formula_binary.txt", "Formula_multi.txt" and "Formula_num.txt"
